...
Section:NewToggle("AutoFarm Coin", "ToggleInfo", function(state)
    if state then
        -- Выключаем прошлую копию этого скрипта перед запуском новой
...
local FLY_SPEED = 22       -- Скорость полета (20-25 безопасно)
local COLLECT_DELAY = 0.25 -- Задержка после сбора

-- Поиск папки с монетами
local function findCoins()
    local container = workspace:FindFirstChild("CoinContainer", true)
    return container and container:FindFirstChild("Coin_Server")
end

-- Безопасный полет без зависаний
local function flyToCoin(root, targetPart)
    if not root or not targetPart or not targetPart.Parent then return false end
    
    local distance = (root.Position - targetPart.Position).Magnitude
    local duration = distance / FLY_SPEED
    
    local tweenInfo = TweenInfo.new(duration, Enum.EasingStyle.Linear)
    local tween = TweenService:Create(root, tweenInfo, {CFrame = targetPart.CFrame * CFrame.new(0, 0.8, 0)})
    
    tween:Play()
    
    local completed = false
    local connection
    connection = tween.Completed:Connect(function() completed = true end)
    
    -- Проверяем состояние во время полета
    while not completed and _G.AutoFarmActive do
        if not root or not root.Parent or not targetPart or not targetPart.Parent then
            tween:Cancel()
    ...
    if connection then connection:Disconnect() end
    return _G.AutoFarmActive
end

print("=== БЕЗОПАСНЫЙ КРУГЛОСУТОЧНЫЙ АВТОФАРМ ЗАПУЩЕН ===")

-- Бесконечный цикл, который никогда не ломает игру
while _G.AutoFarmActive do
    ...
    -- Сброс при лимите в 40 монет
    if humanoid and humanoid.Health > 0 and coinsCollected >= 40 then
        print("Лимит 40 монет! Делаем ресет...")
        humanoid.Health = 0
        coinsCollected = 0
        wait(5)
        continue
    ...
    if coinServer and root and humanoid and humanoid.Health > 0 then
        ...
        for i = 1, #coins do
            ...
            local coin = coins[i]
            if coinsCollected >= 40 or not humanoid or humanoid.Health <= 0 then break end
            if not coinServer or not coinServer.Parent then break end 
            
            if coin:IsA("BasePart") and coin.Parent == coinServer then
                local reached = flyToCoin(root, coin)
                if not reached then continue end
                
                local attempts = 0
                
                -- Сбор монеты с кручением
                while coin.Parent == coinServer and attempts < 4 and _G.AutoFarmActive do
                    if not humanoid or humanoid.Health <= 0 or coinsCollected >= 40 then break end
                    if not root or not root.Parent then break end
                    
                    local rotation = CFrame.Angles(0, math.rad(attempts * 90), 0)
        ...
        -- Обнуляем счетчик между раундами или после смерти
        if coinsCollected > 0 and (not humanoid or humanoid.Health <= 0) then
            coinsCollected = 0
        end
    end
end

print("=== АВТОФАРМ УСПЕШНО ОСТАНОВЛЕН ===")
