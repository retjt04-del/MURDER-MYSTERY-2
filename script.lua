...
Section:NewToggle("W Auto (when auto-farming coins, it is mandatory)", "ToggleInfo", function(state)
    if state then
        local VirtualInputManager = game:GetService("VirtualInputManager")

getgenv().AutoWalkKey = true

task.spawn(function()
    while getgenv().AutoWalkKey do
        -- Имитируем нажатие клавиши W (код клавиши 0x57)
        VirtualInputManager:SendKeyEvent(true, Enum.KeyCode.W, false, game)
        task.wait(0.1)
    end
    -- Отжимаем клавишу, когда скрипт отключают
    VirtualInputManager:SendKeyEvent(false, Enum.KeyCode.W, false, game)
end)

    else
        getgenv().AutoWalk = false
getgenv().AutoWalkKey = false
