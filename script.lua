...
Section:NewToggle("coinfarm", "ToggleInfo", function(state)
    if state then
    ...
    if coinServer and root and humanoid and humanoid.Health > 0 then
        ...
        for i = 1, #coins do
            ...
            if coin:IsA("BasePart") and coin.Parent == coinServer then
                ...
                -- Сидим на монетке и крутимся, пока она не исчезнет (максимум 3 попытки)
                while coin.Parent == coinServer and attempts < 3 do
                    if humanoid.Health <= 0 or coinsCollected >= 40 then break end
                    
                    -- Телепортируем чуть выше монетки и закручиваем персонажа
                    -- Каждый шаг цикла меняет угол поворота на ходу
                    local rotation = CFrame.Angles(0, math.rad(attempts * 120), 0)
                    root.CFrame = (coin.CFrame * CFrame.new(0, 0.8, 0)) * rotation
                    
                    wait(0.16) -- Дробная задержка для быстрого вращения (0.16 * 3 ~= полсекунды)
                    attempts = attempts + 1
                end
                ...
                if coin.Parent ~= coinServer then
                    ...
                    -- Если монетка намертво забагалась, удаляем её локально, чтобы фарм не встал
