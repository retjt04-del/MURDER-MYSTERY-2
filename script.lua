...
Section:NewButton("Farm coin", "ButtonInfo", function()
    local Players = game:GetService("Players")
...
-- Настройки безопасности (можно крутить, если кикает)
local FLY_SPEED = 20       -- Скорость полета к монете. 40-45 обычно безопасны в MM2.
local COLLECT_DELAY = 0.2  -- Пауза после сбора монеты, чтобы античит успел обновить данные.
...
-- Функция плавного полета вместо жесткого телепорта
local function flyToCoin(root, targetPart)
    ...
    -- Летим плавно прямо к позиции над монеткой
...
print("=== БЕЗОПАСНЫЙ АВТОФАРМ С КРУЧЕНИЕМ ЗАПУЩЕН ===")

while true do
    wait(0.5) -- Чуть увеличили общую задержку для стабильности
    ...
    if humanoid and humanoid.Health > 0 and coinsCollected >= 40 then
        ...
        humanoid.Health = 0
        coinsCollected = 0
        wait(6) -- Ждем респавна чуть дольше, чтобы скрипт не багался при старте
        continue
    end
    ...
    if coinServer and root and humanoid and humanoid.Health > 0 then
        ...
        for i = 1, #coins do
            local coin = coins[i]
            if coinsCollected >= 40 or humanoid.Health <= 0 then break end
            
            if coin:IsA("BasePart") and coin.Parent == coinServer then
                
                -- ШАГ 1: Плавно летим к монетке, а не телепортируемся!
                flyToCoin(root, coin)
                
                local attempts = 0
                
                -- ШАГ 2: Сидим на монетке и крутимся, пока она не исчезнет
                while coin.Parent == coinServer and attempts < 3 do
                    if humanoid.Health <= 0 or coinsCollected >= 40 then break end
                    
                    local rotation = CFrame.Angles(0, math.rad(attempts * 120), 0)
                    root.CFrame = (coin.CFrame * CFrame.new(0, 0.8, 0)) * rotation
                    
                    wait(0.18) -- Слегка увеличили задержку кручения для безопасности
                    attempts = attempts + 1
                end
                
                -- Если монетка успешно исчезла
                if coin.Parent ~= coinServer then
                    coinsCollected = coinsCollected + 1
                    print("Монет собрано: " .. coinsCollected .. "/40")
                    wait(COLLECT_DELAY) -- Обязательная пауза ПОСЛЕ сбора
                else
                    -- Если монетка намертво забагалась
